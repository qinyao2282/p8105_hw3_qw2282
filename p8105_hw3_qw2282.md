...
ny_data = ny_noaa %>% 
  ...
  janitor::clean_names() %>% 
  rename(snow_in = snow, tmax_0.1_celcius = tmax, tmin_0.1_celcius = tmin, prcp_0.1_mm = prcp, snwd_mm = snwd) #Rename some columns to add the appropriate units. 
...
From these teo plots, we first notice that
